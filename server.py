...
Flask + memoria (no disco, compatibile Render free)
"""
    ...
    with _lock:
        ...
        if mid in machines:
            machines[mid].update({
                ...
                "rustdesk_id":data.get("rustdesk_id", machines[mid]["rustdesk_id"]),
                "last_seen":  now,
                "status":     "online",
                "cpu":        data.get("cpu", machines[mid]["cpu"]),
                "ram":        data.get("ram", machines[mid]["ram"]),
                "disk":       data.get("disk", machines[mid]["disk"]),
            })
        else:
            machines[mid] = {
                "id":          mid,
...
def create_ticket():
    ...
    data["machine_id"] = mid
    upsert_machine(data, mid)
    t = new_ticket(data)
    return jsonify({"ok": True, "ticket_id": t["id"], "machine_id": mid})
...
def get_stats():
    ...
    with _lock:
        ...
        urgent  = sum(1 for t in tickets if t["status"] == "open" and t["priority"] == "urgent")
        return jsonify({
...
def demo_data():
    now = datetime.now().isoformat()
    demo_machines = [
        {"pc_name":"DESKTOP-MARIO","user":"mario.rossi","domain":"UPTIME.LOCAL","ip":"192.168.1.10","os":"Windows 11 Pro","rustdesk_id":"123 456 789","cpu":45,"ram":62,"disk":78},
        {"pc_name":"LAPTOP-GIULIA","user":"giulia.bianchi","domain":"UPTIME.LOCAL","ip":"192.168.1.11","os":"Windows 10 Pro","rustdesk_id":"987 654 321","cpu":12,"ram":34,"disk":45},
        {"pc_name":"SERVER-FILE","user":"Administrator","domain":"UPTIME.LOCAL","ip":"192.168.1.1","os":"Windows Server 2022","rustdesk_id":"111 222 333","cpu":78,"ram":88,"disk":92},
        {"pc_name":"PC-CONTABILITA","user":"admin","domain":"WORKGROUP","ip":"192.168.1.20","os":"Windows 10 Home","rustdesk_id":"","cpu":0,"ram":0,"disk":0},
    ]
    for d in demo_machines:
        mid = hashlib.md5(f"{d['pc_name']}{d['domain']}".encode()).hexdigest()[:12]
        upsert_machine(d, mid)

    demo_tickets = [
        {"pc_name":"DESKTOP-MARIO","user":"mario.rossi","domain":"UPTIME.LOCAL","ip":"192.168.1.10","rustdesk_id":"123 456 789","description":"Il PC va lento, non riesco ad aprire Excel","priority":"urgent"},
        {"pc_name":"LAPTOP-GIULIA","user":"giulia.bianchi","domain":"UPTIME.LOCAL","ip":"192.168.1.11","rustdesk_id":"987 654 321","description":"Stampante non trovata in rete","priority":"normal"},
        {"pc_name":"SERVER-FILE","user":"Administrator","domain":"UPTIME.LOCAL","ip":"192.168.1.1","rustdesk_id":"111 222 333","description":"Disco quasi pieno - 92% utilizzato","priority":"urgent"},
    ]
    for d in demo_tickets:
        mid = hashlib.md5(f"{d['pc_name']}{d['domain']}".encode()).hexdigest()[:12]
        d["machine_id"] = mid
        new_ticket(d)
